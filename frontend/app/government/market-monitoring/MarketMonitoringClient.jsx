...
import { Search, TrendingUp, AlertTriangle, ScanSearch } from 'lucide-react';
...
// Reused verbatim at /admin/market-monitoring — see app/admin/market-monitoring/page.js.
export default function MarketMonitoringClient() {
  return (
...
function MarketMonitoring() {
  ...
  }, [user, page, category, status, sortBy, refreshToken]);
  ...
  const handleScan = async () => {
    ...
    setError('');
    try {
      const res = await api.post('/market-monitoring/scan');
      setScanResult(res.data);
      setRefreshToken((v) => v + 1);
    } catch (err) {
      setError(err.response?.data?.message || 'Price scan failed');
    } finally {
      setScanning(false);
    }
  };
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="text-right">
          <button
            ...
            onClick={handleScan}
            disabled={scanning}
            className="flex items-center gap-2 rounded-full bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary-dark disabled:opacity-50"
          >
            <ScanSearch size={16} />
            {scanning ? 'Scanning...' : 'Run Price Scan'}
          </button>
          {scanResult && (
            <p className="mt-2 text-xs text-ink-muted">
              Checked {scanResult.productsChecked} products &middot; flagged {scanResult.flagged}
            </p>
          )}
        </div>
      </div>
                  ...
                  {filteredPrices.map((p) => (
                    <tr key={p.priceId}>
                      ...
                      <td className="px-5 py-3 text-right">
                        <Link
                          href={`${basePath}/market-monitoring/${p.priceId}`}
